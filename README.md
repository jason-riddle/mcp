...
The project includes a comprehensive staged documentation generator that automatically updates API documentation:
...
The documentation generator uses a 5-stage process:
1. **Data Extraction**: Uses Java reflection (with regex fallback) to extract `@Tool` and `@Resource` annotations
2. **Content Generation**: Creates properly formatted markdown sections for tools and resources
3. **Staging**: Generates a complete staging README file for validation
4. **Validation**: Validates markdown structure, content integrity, and section markers
5. **Apply Changes**: Creates automatic backups and safely applies changes

This staged approach ensures reliable documentation generation with validation, backups, and rollback capabilities.
