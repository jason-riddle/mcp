...
URL Source: https://quarkus.io/guides/getting-started-testing

Markdown Content:
By Version

[Edit this Page](https://github.com/quarkusio/quarkus/edit/main/docs/src/main/asciidoc/getting-started-testing.adoc)
...
[](https://quarkus.io/guides/getting-started-testing#prerequisites)1. Prerequisites
-----------------------------------------------------------------------------------
...
*   The completed greeter application from the [Getting Started Guide](https://quarkus.io/guides/getting-started)

[](https://quarkus.io/guides/getting-started-testing#architecture)2. Architecture
---------------------------------------------------------------------------------
...
[](https://quarkus.io/guides/getting-started-testing#solution)3. Solution
-------------------------------------------------------------------------

We recommend that you follow the instructions in the next sections and create the application step by step. However, you can go right to the completed example.

The solution is located in the `getting-started-testing`[directory](https://github.com/quarkusio/quarkus-quickstarts/tree/main/getting-started-testing).

This guide assumes you already have the completed application from the `getting-started` directory.

[](https://quarkus.io/guides/getting-started-testing#recap-of-http-based-testing-in-jvm-mode)4. Recap of HTTP based Testing in JVM mode
---------------------------------------------------------------------------------------------------------------------------------------
...
Because we are using JUnit 5, the version of the [Surefire Maven Plugin](https://maven.apache.org/surefire/maven-surefire-plugin/) must be set, as the default version does not support Junit 5:
...
### [](https://quarkus.io/guides/getting-started-testing#controlling-the-test-port)4.1. Controlling the test port
...
### [](https://quarkus.io/guides/getting-started-testing#controlling-http-interaction-timeout)4.2. Controlling HTTP interaction timeout
...
### [](https://quarkus.io/guides/getting-started-testing#injecting-a-uri)4.3. Injecting a URI
...
[](https://quarkus.io/guides/getting-started-testing#testing-a-specific-endpoint)5. Testing a specific endpoint
---------------------------------------------------------------------------------------------------------------
...
### [](https://quarkus.io/guides/getting-started-testing#testhttpresource)5.1. TestHTTPResource
...
### [](https://quarkus.io/guides/getting-started-testing#restassured)5.2. RESTassured
...
[](https://quarkus.io/guides/getting-started-testing#injection-into-tests)6. Injection into tests
-------------------------------------------------------------------------------------------------
...
[](https://quarkus.io/guides/getting-started-testing#tests-and-transactions)8. Tests and Transactions
-----------------------------------------------------------------------------------------------------
...
[](https://quarkus.io/guides/getting-started-testing#enrichment-via-quarkustestcallback)9. Enrichment via QuarkusTest*Callback
------------------------------------------------------------------------------------------------------------------------------
...
[](https://quarkus.io/guides/getting-started-testing#testing_different_profiles)10. Testing Different Profiles
--------------------------------------------------------------------------------------------------------------
...
### [](https://quarkus.io/guides/getting-started-testing#running-specific-tests)10.2. Running specific tests
...
[](https://quarkus.io/guides/getting-started-testing#nested-tests)11. Nested Tests
----------------------------------------------------------------------------------

JUnit 5 [@Nested tests](https://junit.org/junit5/docs/current/user-guide/#writing-tests-nested) are useful for structuring more complex test scenarios. However, note that it is not possible to assign different test profiles or resources to nested tests within the same parent class.

[](https://quarkus.io/guides/getting-started-testing#mock-support)12. Mock Support
----------------------------------------------------------------------------------
...
### [](https://quarkus.io/guides/getting-started-testing#cdi-alternative-mechanism)12.1. CDI `@Alternative` mechanism.
...
### [](https://quarkus.io/guides/getting-started-testing#quarkus_mock)12.2. Mocking using QuarkusMock
...
#### [](https://quarkus.io/guides/getting-started-testing#further-simplification-with-injectmock)12.2.1. Further simplification with `@InjectMock`

Building on the features provided by `QuarkusMock`, Quarkus also allows users to effortlessly take advantage of [Mockito](https://site.mockito.org/) for mocking the beans supported by `QuarkusMock`.
...
#### [](https://quarkus.io/guides/getting-started-testing#using-spies-instead-of-mocks-with-injectspy)12.2.2. Using Spies instead of Mocks with `@InjectSpy`

Building on the features provided by `InjectMock`, Quarkus also allows users to effortlessly take advantage of [Mockito](https://site.mockito.org/) for spying on the beans supported by `QuarkusMock`. This functionality is available via the `@io.quarkus.test.junit.mockito.InjectSpy` annotation which is available in the `quarkus-junit5-mockito` dependency.

Sometimes when testing you only need to verify that a certain logical path was taken, or you only need to stub out a single method’s response while still executing the rest of the methods on the Spied clone. Please see [Mockito documentation - Spying on real objects](https://javadoc.io/doc/org.mockito/mockito-core/latest/org/mockito/Mockito.html#13) for more details on Spy partial mocks. In either of those situations a Spy of the object is preferable. Using `@InjectSpy`, the previous example could be written as follows:
...
#### [](https://quarkus.io/guides/getting-started-testing#using-injectmock-with-restclient)12.2.3. Using `@InjectMock` with `@RestClient`
...
[](https://quarkus.io/guides/getting-started-testing#testing-security)13. Testing Security
------------------------------------------------------------------------------------------

If you are using Quarkus Security, check out the [Testing Security](https://quarkus.io/guides/security-testing) section for information on how to easily test security features of the application.

[](https://quarkus.io/guides/getting-started-testing#quarkus-test-resource)14. Starting services before the Quarkus application starts
--------------------------------------------------------------------------------------------------------------------------------------
...
Quarkus provides a few implementations of `QuarkusTestResourceLifecycleManager` out of the box (see `io.quarkus.test.h2.H2DatabaseTestResource` which starts an H2 database, or `io.quarkus.test.kubernetes.client.KubernetesServerTestResource` which starts a mock Kubernetes API server), but it is common to create custom implementations to address specific application needs. Common cases include starting docker containers using [Testcontainers](https://www.testcontainers.org/) (an example of which can be found [here](https://github.com/quarkusio/quarkus/blob/main/test-framework/keycloak-server/src/main/java/io/quarkus/test/keycloak/server/KeycloakTestResourceLifecycleManager.java)), or starting a mock HTTP server using [Wiremock](https://wiremock.org/) (an example of which can be found [here](https://github.com/geoand/quarkus-test-demo/blob/main/src/test/java/org/acme/getting/started/country/WiremockCountries.java)).
...
### [](https://quarkus.io/guides/getting-started-testing#altering-the-test-class)14.1. Altering the test class
...
### [](https://quarkus.io/guides/getting-started-testing#annotation-based-test-resources)14.2. Annotation-based test resources
...
[](https://quarkus.io/guides/getting-started-testing#hang-detection)15. Hang Detection
--------------------------------------------------------------------------------------
...
[](https://quarkus.io/guides/getting-started-testing#native-executable-testing)16. Native Executable Testing
------------------------------------------------------------------------------------------------------------
...
[](https://quarkus.io/guides/getting-started-testing#quarkus-integration-test)17. Using `@QuarkusIntegrationTest`
-----------------------------------------------------------------------------------------------------------------
...
**1**Use another test runner that starts the application from the native file before the tests. The executable is retrieved by the _Failsafe Maven Plugin_.
...
### [](https://quarkus.io/guides/getting-started-testing#launching-containers)17.1. Launching containers

When `@QuarkusIntegrationTest` results in launching a container (because the application was built with `quarkus.container-image.build` set to `true`), the container is launched on a predictable container network. This facilitates writing integration tests that need to launch services to support the application. This means that `@QuarkusIntegrationTest` works out of the box with containers launched via [Dev Services](https://quarkus.io/guides/dev-services), but it also means that it enables using [QuarkusTestLifecycleManager](https://quarkus.io/guides/getting-started-testing#quarkus-test-resource) resources that launch additional containers. This can be achieved by having your `QuarkusTestLifecycleManager` implement `io.quarkus.test.common.DevServicesContext.ContextAware`. A simple example could be the following:
...
### [](https://quarkus.io/guides/getting-started-testing#executing-against-a-running-application)17.2. Executing against a running application
...
[](https://quarkus.io/guides/getting-started-testing#mixing-quarkustest-with-other-type-of-tests)18. Mixing `@QuarkusTest` with other type of tests
---------------------------------------------------------------------------------------------------------------------------------------------------
...
[](https://quarkus.io/guides/getting-started-testing#test-from-ide)19. Running `@QuarkusTest` from an IDE
---------------------------------------------------------------------------------------------------------
...
*   `java.util.logging.manager` (see [Logging Guide](https://quarkus.io/guides/logging))

*   `maven.home` (only if there are any custom settings in `${maven.home}/conf/settings.xml`, see [Maven Guide](https://quarkus.io/guides/maven-tooling))

*   `maven.settings` (in case a custom version of `settings.xml` file should be used for the tests)

### [](https://quarkus.io/guides/getting-started-testing#eclipse-separate-jre-definition)19.1. Eclipse separate JRE definition
...
### [](https://quarkus.io/guides/getting-started-testing#vscode-run-with-configuration)19.2. VSCode "run with" configuration
...
### [](https://quarkus.io/guides/getting-started-testing#intellij-idea-junit-template)19.3. IntelliJ IDEA JUnit template
...
[](https://quarkus.io/guides/getting-started-testing#testing-dev-services)20. Testing Dev Services
--------------------------------------------------------------------------------------------------

By default, tests should just work with [Dev Services](https://quarkus.io/guides/dev-services), however from some use cases you may need access to the automatically configured properties in your tests.
...
[](https://quarkus.io/guides/getting-started-testing#testing-components)21. Testing Components
----------------------------------------------------------------------------------------------
