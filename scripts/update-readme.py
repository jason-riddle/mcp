...
class ReadmeUpdater:
    ...
    def _update_section(self, content: str, section_type: str, new_content: str) -> str:
        ...
        print(f"[DEBUG] Looking for {section_type} start pattern: {start_pattern}", file=sys.stderr)
        start_match = re.search(start_pattern, content)
        if not start_match:
            print(f"[ERROR] Start marker pattern '{start_pattern}' not found in README", file=sys.stderr)
            # Try to find what's actually there
            lines = content.split('\n')
            for i, line in enumerate(lines):
                if f'{section_type.capitalize()} generated by' in line:
                    print(f"[DEBUG] Found {section_type} marker at line {i+1}: {repr(line)}", file=sys.stderr)
            return content

        print(f"[DEBUG] Found {section_type} start marker at position {start_match.start()}-{start_match.end()}", file=sys.stderr)
        ...
        print(f"[DEBUG] Found {section_type} end marker at position {end_idx}", file=sys.stderr)
