...
class ReadmeUpdater:
    ...
    def _update_tools_section(self, content: str, tools: List[Dict]) -> str:
        ...
        start_marker = "<!--- Tools generated by update-readme.py -->"
        end_marker = "<!--- End of tools generated section -->"

        tools_content = self._generate_tools_content(tools)
        return self._update_section(content, start_marker, end_marker, tools_content)

    def _update_resources_section(self, content: str, resources: List[Dict]) -> str:
        """Update the resources section in README"""
        start_marker = "<!--- Resources generated by update-readme.py -->"
        end_marker = "<!--- End of resources generated section -->"

        resources_content = self._generate_resources_content(resources)
        return self._update_section(content, start_marker, end_marker, resources_content)

    def _update_section(self, content: str, start_marker: str, end_marker: str, new_content: str) -> str:
        """Update a section between markers in the content"""
        start_idx = content.find(start_marker)
        end_idx = content.find(end_marker)

        if start_idx == -1 or end_idx == -1:
            print(f"WARNING: Markers '{start_marker}' or '{end_marker}' not found in README")
            return content
